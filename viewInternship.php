...
<?php
											...
											$result=mysqli_query($con,"select * from tblInternship INNER JOIN tblJobGroup ON tblInternship.JobGroupId = tblJobGroup.JobGroupId INNER JOIN tblCompany ON tblInternship.CompanyId = tblCompany.CompanyId INNER JOIN tblStudent ON tblInternship.StudentId = tblStudent.StudentId") or die("Failed to Query Database".mysqli_error());
													...
													<?php
															  if( mysqli_num_rows( $result )==0 ){
																...
																while( $row = mysqli_fetch_assoc( $result ) ){
																  echo "<tr><td>{$row['StudentId']}</td><td>{$row['StudentFirstName']}</td><td>{$row['StudentLastName']}</td><td>{$row['CompanyName']}</td><td>{$row['InternshipType']}</td><td>{$row['InternshipPay']}</td>
																 <td>
