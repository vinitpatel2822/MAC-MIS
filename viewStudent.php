...
<?php
											...
											$result=mysqli_query($con,"select * from tblStudent INNER JOIN tblSemester ON tblStudent.SemesterId = tblSemester.SemesterId") or die("Failed to Query Database".mysqli_error());
													...
													<?php
															  if( mysqli_num_rows( $result )==0 ){
																...
																while( $row = mysqli_fetch_assoc( $result ) ){
																  echo "<tr><td>{$row['StudentId']}</td><td>{$row['StudentFirstName']}</td><td>{$row['StudentMiddleName']}</td><td>{$row['StudentLastName']}</td><td>{$row['StudentEmail']}</td><td>{$row['Semester']}</td>
																 <td>
