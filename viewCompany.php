							...
							<div class="row">
									<div class="col-xs-12">
										...
										<div class="table-header">
											Results for "Registered Companies"
										</div>
<br/>
										...
										<div>
											...
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												/*		SELECT Orders.OrderID, Customers.CustomerName, Shippers.ShipperName
														...
														$result=mysqli_query($con,"select * from (((tblCompany INNER JOIN tblCountry ON tblCompany.CountryId = tblCountry.CountryId) INNER JOIN tblState ON tblCompany.StateId = tblState.StateId) INNER JOIN tblCity ON tblCompany.CityId = tblCity.CityId) ") or die("Failed to Query Database".mysqli_error());
														//$row=mysql_fetch_array($result);
																...
																while( $row = mysqli_fetch_assoc( $result ) ){
																  echo "<tr><td>{$row['CompanyId']}</td><td>{$row['CompanyName']}</td><td>{$row['CompanyAddress']}</td><td>{$row['CountryName']}</td><td>{$row['StateName']}</td><td>{$row['CityName']}</td><td>{$row['CompanyPostalCode']}</td><td>{$row['CompanyCPFName']}</td><td>{$row['CompanyCPPosition']}</td><td>{$row['CompanyTelephone']}</td><td>{$row['CompanyEmail']}</td><td>{$row['CompanyWebsite']}</td>
																 <td>
