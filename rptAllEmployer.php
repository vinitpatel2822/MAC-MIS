				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">All Employers</li>
						</ul><!-- /.breadcrumb -->
					...
					<div class="page-content">
						...
						<form action="rptAllEmployer.php" class="form-horizontal" role="form" method="POST">
									
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Job Search by Employer: </label>
										...
										<div class="col-sm-3 widget-body">
															...
															<select class="form-control" name="company" id="form-field-select-1">
																...
																$sql = mysqli_query($con, "SELECT * FROM tblJob INNER JOIN tblCompany ON tblJob.CompanyId = tblCompany.CompanyId");
...
<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												/*		SELECT Orders.OrderID, Customers.CustomerName, Shippers.ShipperName
														...
														$result=mysqli_query($con,"select * from ((tblJob INNER JOIN tblCompany ON tblJob.CompanyId = tblCompany.CompanyId) INNER JOIN tblJobGroup ON tblJob.JobGroupId = tblJobGroup.JobGroupId) where tblCompany.CompanyId = '$c_ompany'") or die("Failed to Query Database".mysqli_error());
														//$row=mysql_fetch_array($result);
																...
																while( $row = mysqli_fetch_assoc( $result ) ){
																  echo "<tr><td>{$row['JobId']}</td><td>{$row['CompanyName']}</td><td>{$row['JobGroupName']}</td><td>{$row['InternshipType']}</td><td>{$row['JobPosition']}</td><td>{$row['JobDescription']}</td><td>{$row['JobResponsibilities']}</td><td>{$row['JobRequirements']}</td><td>{$row['JobSalary']}</td><td>{$row['JobStatus']}</td>
																 <td>
